...
/**
 * 已剪除 sessionId 的保留時間,擋重用(spec §6.3、TOMBSTONE_TTL_MS 預設值)。
 * Registry 的 markCut 需要明確 ttl 才能執行有界的 tombstone 清理。
 */
const CUT_TTL_MS = 60_000;
...
export function createWatchdog(deps: WatchdogDeps): {
  ...
  function sweep(nowMs: number): void {
    ...
    for (const session of expiredSessions(registry.getStore(), nowMs, graceMs)) {
      ...
      registry.markCut(session.id, nowMs, CUT_TTL_MS);
