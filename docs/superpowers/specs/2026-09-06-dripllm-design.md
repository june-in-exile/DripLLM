...
用一個可跑的最小系統,示範 **agent payment 的強制執行力**:買方 agent 必須持續付費才能維持 LLM 推論串流,一旦停付,賣方 server **主動切斷連線**。

論述:傳統扣款為「人」設計 —— 金額夠大、有人按確認、事先綁帳號。機器之間的付款是高頻、微額、免帳號、需可程式化管控的。DripLLM 用「停付即斷流」這一個畫面,證明強制權可以握在賣方手上,且完全由程式執行。
